...
async function seedMovies(client) {
  try {
    await client.sql`CREATE EXTENSION IF NOT EXISTS "uuid-ossp"`;
    const createTable =
      await client.sql`CREATE TABLE IF NOT EXISTS movies (user_id UUID NOT NULL, movie_id INT NOT NULL, type VARCHAR(255) NOT NULL, status VARCHAR(255) NOT NULL, date DATE NOT NULL);`;
    console.log(`Created "movies" table`);
    const insertedMovies = await Promise.all(
      movies.map(
        (movie) =>
          client.sql`INSERT INTO movies (user_id, movie_id, type, status, date) VALUES (${movie.user_id}, ${movie.movie_id}, ${movie.type}, ${movie.status}, ${movie.date});`
      )
    );
